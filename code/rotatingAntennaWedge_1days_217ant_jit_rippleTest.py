...
for dNum in range(nDays):
    ...
    for iDish in range(nDish):
        for jDish in range(iDish):
            ...
            dIndex+=1
    rippleDelay=cosmology.kpara2eta(0.2*0.68,cosmology.f2z(f0))
    for fNum in range(nChan):
        ...
        visList=visList*(1.+1e-3*np.cos(fAxis[fNum]*2.*np.pi*rippleDelay)+1e-2*np.cos(fAxis[fNum]*8.*np.pi*rippleDelay))
        #end=time.time()
...
plt.savefig('wedgeSingleSource_1day_217Ant_rippleTest.png')
#plt.close()
#plt.close()

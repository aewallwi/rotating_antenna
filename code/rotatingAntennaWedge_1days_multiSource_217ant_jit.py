...
#draw poisson sources
#area is the area of the field in Sr
...
flux,spIndices=drawSources(2*np.pi,20.)#draw sources greater than 1 Jy from 2 pi sr
nsrcs=len(flux)
...
maxUV=uvAmp.max()
gridSize=int(np.ceil(maxUV*2./gridSpace))
...
#np.savez('visCubes/sources_nAnt_%d_dAnt_%d_dAntPos_%d.npz'%(nDish,dAnt,dAntPos),l=lAngleList,m=mAngleList,flux=flux,alpha=spIndices)
...
for dNum in range(nDays):
    ...
    for fNum in range(nChan):
        ...
        fluxList=flux*(fAxis[fNum]/150e6)**(-spIndices)
        computeVisList(lAngleList,mAngleList,antDiffP[:,0]/wL,antDiffP[:,1]/wL,fluxList*gainList,visList)
        visCube.append(visList)
        nKernel=int(np.ceil(4.*dAnt/wL/gridSpace))
        uvCube[:,:,fNum]+=gridDishKernel(gridSize,gridSize,gridSpace,gridSpace,antDiffP[:,0]/wL,antDiffP[:,1]/wL,visList,dAnt/2./wL,uvCube[:,:,fNum].squeeze(),len(visList),nKernel)
        sampleCube[:,:,fNum]+=gridDishKernel(gridSize,gridSize,gridSpace,gridSpace,antDiffP[:,0]/wL,antDiffP[:,1]/wL,np.ones(len(visList)),dAnt/2./wL,sampleCube[:,:,fNum].squeeze(),len(visList),nKernel)
#    np.savez('visCubes/visCube_nAnt_%d_dAnt_%d_dAntPos_%d_day_%d.npz'%(nDish,dAnt,dAntPos,dNum),data=np.array(visCube),u=antDiffP[:,0],v=antDiffP[:,1],f=fAxis)
...
np.savez('uvCube_1day_217ant_multiSource_defRes.npz',dCube=uvCubeDelayTransform,du=gridSpace,df=df,f0=f0)
wedgeGrid=delayGrid(np.abs(uvCubeDelayTransform)**2.,gridSpace,maxUV,nKperpBins)
...
plt.savefig('wedgeMultiSource_%d_day_%d_dAnt_%d_dAntPos.png'%(nDays,dAnt,dAntPos))
#plt.close()
#plt.close()
